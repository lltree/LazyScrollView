...
- (void)assembleSubviews:(BOOL)isReload minY:(CGFloat)minY maxY:(CGFloat)maxY
{
    ...
    NSSet<TMLazyItemModel *> *newVisibleModels = [_modelBucket showingModelsFrom:minY - LazyBufferHeight to:maxY + LazyBufferHeight];
...
- (void)reloadData
{
    [self storeItemModelsFromIndex:0];//存ItemModel 到 ModelBucket中
